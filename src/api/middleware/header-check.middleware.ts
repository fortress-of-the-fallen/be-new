...
export class HeaderCheckMiddleware implements NestMiddleware {
   private readonly logger = new Logger(HeaderCheckMiddleware.name);
   ...
   private async isValidUserAgent(userAgent: string): Promise<[boolean, string]> {
      ...
      const userAgentHash = HashHelper.hashString(userAgent);
      if ((await this.redis.exists(userAgentHash)) === 1) {
         this.logger.debug(`User-Agent is cached: ${userAgentHash}`);
         await this.redis.set(userAgentHash, JSON.stringify(true), 'EX', 15);
         return [true, ''];
      }
      ...
      await this.redis.set(userAgentHash, JSON.stringify(true), 'EX', 25);
      return [true, ''];
   }
}
